...
class _SafaMarwaHomePageState extends ConsumerState<SafaMarwaPage> {
  ...
  Widget build(BuildContext context) {
    final provider = ref.watch(safaMarwaProvider);
    return Background(
      ...
      child: Column(
        children: [
                    ...
                    child: SizedBox(
                      ...
                      child: LayoutBuilder(
                        builder: (context, innerConstraints) {
                          return Stack(
                            children: [
                              _SafaMarwaCircle(height: innerConstraints.maxHeight * 0.9, saiRoundCount: provider.saiRoundCount),
                              Positioned(bottom: 0, left: 0, right: 0, child: _SafaMarwaCountButton(onTap: provider.onCountTap, onLongPress: provider.updateRoundCount)),
                            ],
                          );
                        },
                      ),
          ...
          _SafaMarwaDuaSection(saiRoundCount: provider.saiRoundCount, gender: ref.read(umrahProvider.notifier).user?.gender),
        ],
      ),
    );
  }
...
class _SafaMarwaCircle extends StatelessWidget {
  const _SafaMarwaCircle({required this.height, required this.saiRoundCount});
  final double height;
        ...
        child: Align(
          ...
          child: FittedBox(
            child: Column(
              ...
              children: [
                Text(saiRoundCount.toString(), style: CTextStyle.w600(fontSize: 90, color: CColors.primary)),
                Text('${LocaleKeys.completed.tr()}${isLTR(context) ? ' ' : '                     '}${LocaleKeys.round.tr()}', style: CTextStyle.w900(fontSize: 20)),
              ],
            ),
          ),
        ),
...
class _SafaMarwaCountButton extends StatelessWidget {
  const _SafaMarwaCountButton({required this.onTap, this.onLongPress});
...
class _SafaMarwaDuaSection extends StatelessWidget {
  const _SafaMarwaDuaSection({required this.saiRoundCount, this.gender});
  final int saiRoundCount;
  final String? gender;

  String get _duaTitle => switch (saiRoundCount) {
  ...
  String get _dua => switch (saiRoundCount) {
    0 => Dua.round1.dua,
    1 => Dua.round2.dua,
    2 => Dua.round3.dua,
    3 => Dua.round4.dua,
    ...
    return Column(
      ...
      children: [
        ...
        BasicCard(
          ...
          child: Center(child: Text(_dua, style: CTextStyle.w500(fontSize: 16, color: CColors.deepTeal, fontFamily: saiRoundCount < 6 ? Helper.arabicTextFontFamily : null), textAlign: TextAlign.center, textDirection: TextDirection.rtl)),
        ),
      ],
    );
  }
